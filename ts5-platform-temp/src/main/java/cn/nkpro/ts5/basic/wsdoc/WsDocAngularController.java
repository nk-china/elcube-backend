	...
	private List<Fun> methods(Class<?> controllerType){
		...
		return Arrays
				...
				.map(method->{
					...
					WsDocNote methodNote = method.getAnnotation(WsDocNote.class);
					// NOTES
	...
	public Map<String, Object> method(
		...
		if(methodNote!=null) {
			...
			methodConfig.put("dependentEnums", Arrays.stream(methodNote.dependentEnums())
					.map(clazz -> {
						WsDocNote codeNotes = clazz.getAnnotation(WsDocNote.class);
		...
		if (exceptions.length > 0) {
			...
			for (Class<?> exception : exceptions) {
				WsDocNote eNotes = exception.getAnnotation(WsDocNote.class);
				if (eNotes != null) {
		...
		for (int i = 0; i < types.length; i++) {
			...
			for (Annotation anno : annos) {
				if (anno.annotationType() == WsDocNote.class) {
					if (paramConfig == null) {
						paramConfig = new HashMap<>();
